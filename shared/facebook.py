...
# TODO(Sandy): Rename function names to be less ambiguous
def get_fb_token(code, app):
    """
...
def get_fb_long_token(short_token, app):
    """
...
def get_fb_data(signed_request, app):
    ...
    # Validated. Fetch long tokenb from facebook
    ...
    if code:
        ...
        result_dict = get_fb_token(code, app)

        short_access_token = result_dict.get('access_token')
        if short_access_token:

            result_dict = get_fb_long_token(short_access_token, app)
